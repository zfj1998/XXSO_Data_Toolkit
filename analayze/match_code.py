...
match_count=0
not_match_count=0
match_file_path='./match-pysrc-train.json'
notmatch_file_path='./notmatch-pysrc-train.json'
...
with open(match_file_path,mode="w",encoding="utf-8") as mfw:
    with open(notmatch_file_path,mode="w",encoding="utf-8") as nmfw:
        for line in f.readlines():
            ...
            topone=i['top_three_simple'][0]['body'].lower()#全部转换为小写，因为夏新的就是小写
            topone1=topone.replace(" ","").replace("'","").replace("\"","").replace("``","").replace("@","").replace("\n","")
            topone1=topone1.split('<code>')[1].split('</code>')[0]
            #调试，观察输出
            print(topone1)
            break
